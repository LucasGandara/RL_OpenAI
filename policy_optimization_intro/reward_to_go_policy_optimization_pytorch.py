...
def train_one_epoch(env, batch_size, logits_net, optimizer):
    batch_obs, batch_acts, batch_weights, batch_rets, batch_lens = collect_experience(env=env, batch_size=batch_size, logits_net=logits_net)
...
def train(env_name='CartPole-v1', hidden_sizes=[32], lr=1e-2, 
    ...
    for i in range(epochs):
        batch_loss, batch_rets, batch_lens = train_one_epoch(env=env, batch_size=batch_size, logits_net=logits_net, optimizer=optimizer)
        print('epoch: %3d \t loss: %.3f \t return: %.3f \t ep_len: %.3f'%
                (i, batch_loss, np.mean(batch_rets), np.mean(batch_lens)))

    env = gymnasium.make(env_name, render_mode='human')
    batch_loss, batch_rets, batch_lens = train_one_epoch(env=env, batch_size=1, logits_net=logits_net, optimizer=optimizer)
