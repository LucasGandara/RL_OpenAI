...
def create_q_model():
    ...
    return keras.Sequential(
        [
            ...
            layers.Conv2D(64, 4, strides=2, activation="relu", name="conv2"),
            layers.Conv2D(64, 3, strides=1, activation="relu", name="conv3"),
            layers.Flatten(),
...
model_target = Model(num_actions)
...
while True:
    ...
    for timestep in range(1, max_steps_per_episode):
        ...
        if (
            False
        ):  # frame_count < epsilon_random_frames or epsilon > np.random.rand(1)[0]:
            # Take random action
