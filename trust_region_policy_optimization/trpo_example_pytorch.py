...
def train(epochs=100, num_rollouts=10, render_frequency=None):
    ...
    for epoch in range(epochs):
        ...
        for t in range(num_rollouts):
            ...
            while not done:
                if (
                    render_frequency is not None
                    and global_rollout % render_frequency == 0
                ):
                    pass
...
def get_action(state_input):
    state = (
        torch.tensor(state_input, dtype=torch.float64).float().unsqueeze(0)
    )  # Turn state into a batch with a single element
    probs = torch.nan_to_num(actor(state))
    try:
        dist = Categorical(
            probs
...
critic = nn.Sequential(
    nn.Linear(state_size, critic_hidden), nn.ReLU(), nn.Linear(critic_hidden, 1)
)
...
def update_agent(rollouts):
    ...
    d_kl = flat_grad(
        KL, parameters, create_graph=True
...
# Train our agent
train(epochs=7, num_rollouts=args.num_rollouts, render_frequency=250)
