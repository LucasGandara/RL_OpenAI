...
from common import CNN, MLP, CreateInputLayer
...
class ValueNetwork(Model):
    def __init__(
        ...
        with tf.name_scope("Output"):
            outputs = Dense(n_units=1, act=output_activation, W_init=w_init)(
                current_layer
            )
    ...
    def __call__(self, states, *args, **kwargs):
        if isinstance(self._state_space, gym.spaces.Dict):
            ...
            if np.shape(states)[1:] != self.state_shape:
                raise ValueError(
                    "Input state shape error. Shape can be {} but your shape is {}".format(
...
class QNetwork(Model):
    def __init__(
        ...
        with tf.name_scope("Outputs"):
            if isinstance(self._action_space, gym.spaces.Discrete):
                if self.dueling:
                    ...
                    q = Dense(
                        n_units=self._action_shape[0],
                        act=output_activation,
                        W_init=w_init,
                    )(current_layer)
                    mean_q = tl.layers.Lambda(lambda x: tf.reduce_mean(x, 1, True))(q)
                    current_layer = tl.layers.Lambda(lambda x: x[0] + x[1] - x[2])(
                        (v, q, mean_q)
                    )
                else:
                    current_layer = Dense(
                ...
                if not self.state_only:
                    act_one_hot = tl.layers.OneHot(depth=self._action_shape[0], axis=1)(
                        act_inputs
                    )  # discrete action choice to one-hot vector
                    outputs = tl.layers.Lambda(
                        lambda x: tf.reduce_sum(tf.reduce_prod(x, axis=0), axis=1)
                    )((current_layer, act_one_hot))
                else:
        ...
        if isinstance(state_space, gym.spaces.Dict):
            if self.state_only:
                ...
                super().__init__(
                    inputs=obs_inputs + [act_inputs], outputs=outputs, name=name
                )
        else:
            if self.state_only:
                super().__init__(inputs=obs_inputs, outputs=outputs, name=name)
    ...
    def __call__(self, inputs, *args, **kwargs):
        ...
        if isinstance(self._state_space, gym.spaces.Dict):
            states = (
                np.array(states).transpose([1, 0]).tolist()
            )  # batch states to multi-head
        ...
        if not self.state_only:
            if isinstance(self._action_space, gym.spaces.Discrete) and np.any(
                actions % 1
            ):
...
class NAFLayer(tl.layers.Layer):
    ...
    def forward(self, inputs):
        ...
        adv = tf.squeeze(
            -0.5 * tf.matmul(tf.transpose(temp, [0, 2, 1]), tf.matmul(P, temp)), -1
        )
        return adv + value
...
class NAFQNetwork(Model):
    def __init__(
        ...
        with tf.name_scope("NAF_VALUE_MLP"):
            ...
            value = Dense(n_units=1, W_init=w_init, name="naf_value_mlp_output")(
                current_layer
            )
        ...
        with tf.name_scope("NAF_ADVANTAGE"):
            mu = Dense(n_units=dim, act=activation, W_init=w_init, name="mu")(
                current_layer
            )
            L = Dense(n_units=int((dim * (dim + 1)) / 2), W_init=w_init, name="L")(
                current_layer
            )
